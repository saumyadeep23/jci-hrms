...
- `TerminalSettlementController.approve` — `SUPER_ADMIN` removed, `HR_ADMIN` retained. **REQUIRES_BUSINESS_CONFIRMATION unresolved**: the RBAC permission matrix has no distinct checker permission for terminal settlement (unlike CPF/JCIECCS/Payroll's own `*_ADMIN`/`*_APPROVE` roles) — whether approval should require a role distinct from whoever generates the settlement (maker != checker) is not decided and not invented here. Verified: `TerminalSettlementControllerSecurityTest` (SUPER_ADMIN-only → 403; HR_ADMIN → reaches the service).
- `PayrollMasterController.updateSalaryHead`/`updateStatutoryHead`/`reviseStatutoryParameter` — `SUPER_ADMIN` removed, `HR_ADMIN`/`FINANCE_ADMIN` (whichever was already listed per endpoint) retained. **REQUIRES_BUSINESS_CONFIRMATION unresolved**: `HR_ADMIN_BILL`/`HR_MAKER_BILL` only carry `PAYROLL_VIEW`/`PREPARE`/`FINALIZE`/`REVERSE` — no seeded permission for master-data edits exists, so which role should ultimately own this is not decided. Verified: `PayrollMasterControllerTest.updateSalaryHead_withOnlySuperAdminRole_returns403`.
...
- `StateMasterController`/`DistrictMasterController` — `hasRole('SUPER_ADMIN')` is the **only** role granting these two master-data write endpoints; no other role (e.g. `HR_ADMIN`) is currently listed anywhere as an intended alternate for state/district masters specifically. Removing `SUPER_ADMIN` here with no replacement would be a functional regression (nobody could edit these masters); adding `HR_ADMIN` would be inventing a business rule about who should own geographic master data. Left as-is — REQUIRES_BUSINESS_CONFIRMATION. (Pre-existing, separately tracked inconsistency: SEC-011 already flags that `MasterStateController`, a different controller over similar data, uses `isAuthenticated()` instead — not resolved by this pass either.)
...
**Not claimed complete:** SEC-010 remains recorded as "COMPLIANT — financial authorization boundary. Legacy non-financial RBAC migration remains open" (see `VAPT_READINESS_REGISTER.md`) — the 3 REQUIRES_BUSINESS_CONFIRMATION items above, the 2 State/District master items, and the still-reserved `HR_ADMIN_PERS`/`HR_ADMIN_EST`/`IT_ADMIN_STORE` permission families (no live endpoint to migrate) are genuinely open, not silently closed.
